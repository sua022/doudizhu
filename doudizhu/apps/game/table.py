...
class Table(object):
    ...
    def __init__(self, uid, on_table_change: Callable, allow_robot=True):
        self.uid = uid
    ...
    async def handle_chat(self, player, msg):
    ...
    async def on_game_over(self, winner):
        # if winner.hand_pokers:
        #     return
        coin = self.call_score * self.multiple
        for p in self.players:
    ...
    def __str__(self):
        return f'[{self.uid}-{self.players}]'
